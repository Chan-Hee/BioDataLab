...
file_ori <- read.csv("FinalData_GSM_gene_index_result_random.csv",header=TRUE) 
#file_ori <- read.csv("FinalData_GSM_gene_index_result_random.csv",header=TRUE)
...
for(i in conf){
  ...
  file_name <- paste0("FinalData_Random6000_Random_", i, "off_GSM_gene_index_result.csv", collapse = NULL)
