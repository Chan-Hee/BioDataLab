...
#save path
                if(max(AccuracyList)> max_Accuracy):
                    ...
                    save_path = saver.save(sess, '/home/tjahn/tf_save_data/kyulhee/save_path2/saved')
        ...
        weighted_abs_sum = np.absolute(w1_matrix).sum(axis=1)
        weighted_abs_sum = weighted_abs_sum.tolist()
    ...
    return train_p ,train_h, test_p,test_h,weighted_sum_result,gene_names,weighted_abs_sum
...
####sm
    ...
    datafilename = "/home/tjahn/Data/FinalData95off_GSM_gene_index_result.csv"
...
####sm
  ...
  #  Max_Accuracy = []
    ...
    ####Cal Data Set#####
        ...
        train_p, train_h , test_p ,test_h, weighted_sum_result, gene_names, weighted_abs_sum = (set_train_three_layer(nodes, learning_rate, j, gene_off))

        if(j==0):
            abs_sum_list=weighted_abs_sum
        else:
            abs_sum_list=[abs_sum_list + weighted_abs_sum for abs_sum_list, weighted_abs_sum in zip(abs_sum_list, weighted_abs_sum)]
      ...
      #  Max_Accuracy.append(weighted_sum_result[5])
        abs_file_name = "abs_w_sum_sample_"+str(j)+".csv"
        abs_sum_matrix = pd.DataFrame(data={"names": gene_names, "abs_w_sum": weighted_abs_sum})
        abs_sum_matrix.to_csv(output_directory+abs_file_name,sep=",")
        
        print("weighted_abs_sum:\n",weighted_abs_sum)

## abs_sum_list
    abs_sum_avg = [x / 5 for x in abs_sum_list]    
    abs_sum_avg_matrix = pd.DataFrame(data={"names": gene_names, "abs_w_sum_avg": abs_sum_avg})
    abs_sum_avg_matrix.to_csv(output_directory+"abs_weight_sum_sample.csv",sep=",")
