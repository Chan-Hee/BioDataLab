...
#save path
    ...
    return train_p ,train_h, test_p,test_h,weighted_sum_result,gene_names,weighted_sum
...
####sm
  ...
  #  Max_Accuracy = []
    ...
    ####Cal Data Set#####
        ...
        train_p, train_h , test_p ,test_h, weighted_sum_result, gene_names, weighted_sum = (set_train_three_layer(nodes, learning_rate, j, gene_off))

        if(j==0):
            sum_list=weighted_sum
        else:
            sum_list=[sum_list + weighted_sum for sum_list, weighted_sum in zip(sum_list, weighted_sum)]
...
## sum_list
    sum_avg = [x / 5 for x in sum_list]    
    sum_avg_matrix = pd.DataFrame(data={"names": gene_names, "w_sum_avg": sum_avg})
    sum_avg_matrix.to_csv(output_directory+"weight_sum.csv",sep=",")
