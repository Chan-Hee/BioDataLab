...
multmerge = function(mypath){
  ...
  datalist = mclapply(filenames, function(x){read.table(file=x,header=T)})
  
  check<-mclapply(datalist,dim)
  index<-unlist(mclapply(check,function(x){x[1]==54675&&x[2]>3}))
  
  datalist<-datalist[index]
  filelist<-filenames[index]
  
  cancer_or_not<-unlist(mclapply(filelist,function(x){as.integer(substr(x,nchar(x)-4,nchar(x)-4))}))
  num_of_patients<-unlist(mclapply(datalist,function(x){dim(x)[2]-3}))
  
  result<-mcmapply(function(x,y){rep(x,y)},cancer_or_not,num_of_patients)
