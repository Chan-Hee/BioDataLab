...
ov_sample = samples[ which(samples$CANCER_CODE=="OV"), ]
ov_sample_ids = as.vector( unique( ov_sample$GSM_ID ) )
...
for( i in 1:length(files) ){
    ...
    tmp = data.frame( x[,which(colnames(x)%in%ov_sample_ids) ] )
...
write.table(collect, file="GPL570_ov.txt", sep="\t", quote=F, row.names=F)

data = collect
data = data[ , colSums(is.na(data)) == 0]

write.table(collect, file="GPL570_ov_na_filttered.txt", sep="\t", quote=F, row.names=F)
...
write.table(data_, file="GPL570_ov_na_filttered_log2.txt", sep="\t", quote=F, row.names=F)
...
write.table(data_n, file="GPL570_ov_na_filttered_log2_normlized.txt", sep="\t", quote=F, row.names=F)
