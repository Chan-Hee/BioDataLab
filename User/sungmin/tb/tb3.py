...
def set_train_three_layer(num,repeat, nodes, learning_rate):
    ...
    cost_summ = tf.summary.scalar("cost_2000_1000_50",cost)
    ...
    accuracy_summ = tf.summary.scalar("accuarcy_2000_1000_50",accuracy)
    with tf.Session() as sess:
	 # tensorboard --logdir=./logs/xor_logs
        merged_summary = tf.summary.merge_all()
        writer = tf.summary.FileWriter("./log/2000_1000_50")
...
def set_train_four_layer(num ,repeat, nodes, learning_rate):
    ...
    cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=hypothesis, labels=Y))
    train = tf.train.AdamOptimizer(learning_rate=learning_rate).minimize(cost)
    cost_summ = tf.summary.scalar("cost_2000_1000_500_50",cost)
    ...
    correct_prediction = tf.equal(predicted,tf.argmax(Y,1))
    accuracy = tf.reduce_mean(tf.cast(correct_prediction, dtype=tf.float32))
    accuracy_summ= tf.summary.scalar("accuracy_2000_1000_500_50",accuracy)
    with tf.Session() as sess:

         # tensorboard --logdir=./logs/xor_logs
        merged_summary = tf.summary.merge_all()
        writer = tf.summary.FileWriter("./log/2000_1000_500_50")
        writer.add_graph(sess.graph)  # Show the graph
...
conf_sungmin = '/home/tjahn/Git/User/sungmin/input/tb3.csv'
...
conf.to_csv('/home/tjahn/Git/User/sungmin/output/result_tb3.csv' , sep= ',')
