...
def set_train_three_layer(num,repeat, nodes, learning_rate,j):
    batch_size = 1000
    ...
    accuracy_summ = tf.summary.scalar(node+"_accuracy",accuracy)
    j=j+1
    js = str(j)
 
    with tf.Session() as sess:
        merged_summary = tf.summary.merge_all()
        writer = tf.summary.FileWriter("./log1/98/" + node+"_"+js)
...
def set_train_four_layer(num ,repeat, nodes, learning_rate,j):
    batch_size = 1000
    tf.reset_default_graph()
    keep_prob = tf.placeholder(tf.float32)
...
conf_sungmin = '/home/tjahn/Git/User/sungmin/final/input1.csv'
...
for i in range(len(conf)):
    ...
    for j in range(5):
        ...
        if(conf.iloc[i]['layer'] == 3):
            train_acc , test_acc = (set_train_three_layer(i,repeat, nodes, learning_rate,j))
            train_accs_conf.append(train_acc)
            test_accs_conf.append(test_acc)
        elif(conf.iloc[i]['layer']== 4):
            train_acc , test_acc = (set_train_four_layer(i,repeat, nodes, learning_rate,j))
            train_accs_conf.append(train_acc)
            test_accs_conf.append(test_acc)
...
conf.to_csv( '/home/tjahn/Git/User/sungmin/final/output/result/98_result.csv' , sep= ',')
