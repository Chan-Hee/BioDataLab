...
def set_train_three_layer(num,repeat, nodes, learning_rate,j,train_rate):
    batch_size = 1000
    ...
    trainc = str(train_rate)

    # cost/loss function
    cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=hypothesis, labels=Y))
    train = tf.train.AdamOptimizer(learning_rate=learning_rate).minimize(cost)
    cost_summ = tf.summary.scalar(trainc+"_"+node+"_cost",cost)
    ...
    accuracy_summ = tf.summary.scalar(trainc+"_"+node+"_accuracy",accuracy)
    j=j+1
    js = str(j)
    cn = str(nodes)
    ...
    with tf.Session() as sess:
        ...
        for step in range(repeat):
            ...
            for i in range(total_num):
                ...
                summary,_=sess.run([merged_summary,train], feed_dict={X: batch_x, Y: batch_y , keep_prob : 0.7})
...
def set_train_four_layer(num ,repeat, nodes, learning_rate,j,train_rate):
    batch_size = 1000
    tf.reset_default_graph()
    keep_prob = tf.placeholder(tf.float32)
...
for i in range(len(conf)):
    repeat, layer, node , learning_rate, gene,train_rate = conf.iloc[i]
    ...
    for j in range(5):
        ...
        if(conf.iloc[i]['layer'] == 3):
            train_acc , test_acc = (set_train_three_layer(i,repeat, nodes, learning_rate,j,train_rate))
            train_accs_conf.append(train_acc)
            test_accs_conf.append(test_acc)
        elif(conf.iloc[i]['layer']== 4):
            train_acc , test_acc = (set_train_four_layer(i,repeat, nodes, learning_rate,j,train_rate))
            train_accs_conf.append(train_acc)
            test_accs_conf.append(test_acc)
