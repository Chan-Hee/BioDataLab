...
def set_train_three_layer(num,repeat, nodes, learning_rate):
    ...
    accuracy_summ = tf.summary.scalar("accuarcy_1000_500_50",accuracy)
    with tf.Session() as sess:
	 # tensorboard --logdir=./logs/xor_logs
        merged_summary = tf.summary.merge_all()
        writer = tf.summary.FileWriter("./log/1000_500_50")
...
def set_train_four_layer(num ,repeat, nodes, learning_rate):
    ...
    correct_prediction = tf.equal(predicted,tf.argmax(Y,1))
    accuracy = tf.reduce_mean(tf.cast(correct_prediction, dtype=tf.float32))
    accuracy_summ= tf.summary.scalar("accuracy_1000_500_500_50",accuracy)
    with tf.Session() as sess:

         # tensorboard --logdir=./logs/xor_logs
        merged_summary = tf.summary.merge_all()
        writer = tf.summary.FileWriter("./log/1000_500_500_50")
        writer.add_graph(sess.graph)  # Show the graph
...
conf_sungmin = '/home/tjahn/Git/User/sungmin/input/tb1.csv'
...
conf.to_csv('/home/tjahn/Git/User/sungmin/output/result_tb1.csv' , sep= ',')
