...
def set_train_three_layer(num,repeat, nodes, learning_rate):
    ...
    cost_summ = tf.summary.scalar(nodes_name + "_cost",cost)
    ...
    accuracy_summ = tf.summary.scalar(nodes_name + "_accuarcy",accuracy)
    with tf.Session() as sess:
	 # tensorboard --logdir=./logs/xor_logs
        merged_summary = tf.summary.merge_all()
        writer = tf.summary.FileWriter("./log/" + nodes_name)
...
def set_train_four_layer(num ,repeat, nodes, learning_rate):
    ...
    correct_prediction = tf.equal(predicted,tf.argmax(Y,1))
    accuracy = tf.reduce_mean(tf.cast(correct_prediction, dtype=tf.float32))
    accuracy_summ= tf.summary.scalar(nodes_name + "_accuracy",accuracy)
    with tf.Session() as sess:
...
conf_sungmin = '/home/tjahn/Git/User/sungmin/rtb/input/1.csv'
...
conf.to_csv('/home/tjahn/Git/User/sungmin/rtb/output/'+nodes_name , sep= ',')
